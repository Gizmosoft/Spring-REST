...
public class EmployeeDAOWrapper {
	...
	public List<EmployeeBean>getEmployeeDetailsWithinSalaryRange(double min, double max){
		List<EmployeeBean> list = new ArrayList<EmployeeBean>(); 
		
		Iterable<EmployeeEntity> listEn= dao.getEmployeeDetailsWithinSalaryRange(min, max);
		listEn.forEach(x->{
			EmployeeBean employee = new EmployeeBean();
			BeanUtils.copyProperties(x, employee);
			list.add(employee);
